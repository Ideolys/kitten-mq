...
function kittenMQ () {
  ...
  let _listenSocket = function listenSocket (host, packet) {
    ...
    let _info = { channel : packet.headers.channel };

    if (packet.headers.error) {
      return _handlers[_handlerId].call(null, packet.headers.error, null, _info);
    }

    _handlers[_handlerId].call(null, null, packet.data, _info);
  }
