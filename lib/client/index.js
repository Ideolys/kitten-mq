...
function kittenMQ () {
   ...
   */
  let _ack = function ack (channel, handlerId, messageId, isAck = true, time = 0) {
    _hosts.broadcast(_config.clientId, _config.privateKey, isAck ? routes.QUEUE_ACK : routes.QUEUE_NACK, handlerId, { channel, time }, messageId);
  };
   ...
   */
  function _verifyChannelArgument (channel) {
    if (channel.ids && channel.version && channel.endpoint) {
      return;
    }

    throw new Error('Channel argument is not correct');
  }
  ...
  function listen (channel, callback, nbRetry = 0) {
    ...
    if (typeof channel === 'string') {
      ...
      _verifyChannelArgument(_handlers[_id].channel);
    }
   ...
   * @example /endpoint_1/*
   * @example { endpoint : String, version : String, ids : Array }
   * @param {Function} callback (err, data, ack, info)
   * @returns {Object} { addId : Function, removeId : Function }
   */
  function consume (channel, callback, nbRetry = 0) {
    ...
    _handlers[_id].callback = (err, data, done, info) => {
       ...
       * Ack or Nack
       * @param {Boolean} isAck true = ack, false = nack
       * @param {Int} time in seconds to delay redeliver of the message
       */
      let _done = function (isAck = true, time = 0) {
        done.call(null, info.channel, _id, info.messageId, isAck, time);
      };
