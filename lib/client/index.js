...
function kittenMQ () {
  ...
  let _ack = function ack (channel, handlerId, messageId, isAck = true) {
    hosts.broadcast(_config.clientId, _config.privateKey, _hosts.sockets, isAck ? routes.QUEUE_ACK : routes.QUEUE_NACK, handlerId, channel, messageId);
  };
  ...
  function connect (config, callback) {
    ...
    utils.getPrivateAndPublicKeys(_config.keysDirectory, _config.keysName, (keys) => {
      ...
      hosts.connect(_config, _listenSocket, (connectedHosts) => {
        ...
        hosts.broadcast(_config.clientId, _config.privateKey, _hosts.sockets, routes.REGISTER_CLIENT, null, keys.public);
        callback();
      });
    });
  }
  ...
  function send (channel, data, callback) {
    ...
    hosts.broadcast(_config.clientId, _config.privateKey, _hosts.sockets, routes.QUEUE_SEND, _id, { channel, data });
  }
  ...
  function listen (channel, callback) {
    ...
    hosts.broadcast(_config.clientId, _config.privateKey, _hosts.sockets, routes.QUEUE_LISTEN, _id, channel);
  }
  ...
  function consume (channel, callback) {
    ...
    hosts.broadcast(_config.clientId, _config.privateKey, _hosts.sockets, routes.QUEUE_CONSUME, _id, channel);
  }
