...
function kittenMQ () {
   ...
   * @param {Int} messageId
   */
  let _ack = function ack (channel, handlerId, messageId) {
    hosts.broadcast(_hosts.sockets, routes.QUEUE_ACK, handlerId, channel, messageId);
  };
  ...
  let _listenSocket = function listenSocket (host, packet) {
    ...
    let _info = { channel : packet.headers.channel, messageId : packet.headers.messageId };
  ...
  function send (channel, data, callback) {
    ...
    hosts.broadcast(_hosts.sockets, routes.QUEUE_SEND, _id, { channel, data });
  }
  ...
  function consume (channel, callback) {
    ...
    // Register handler
    _handlers[_id] = (err, data, done, info) => {
      let _done = (isRequeue = true) => {
        done.call(null, channel, _id, info.messageId);
      };

      callback.call(null, err, data, _done, info);
    };

    hosts.broadcast(_hosts.sockets, routes.QUEUE_CONSUME, _id, channel);
  }
