...
module.exports = {
   ...
   * @param {Object} config
   */
  start : function (config, clients, publicKeys, listenerBroker, listenerManagement) {
    ...
    _socketServer.on('message', (packet, client) => {
      ...
      if (!packet.data.headers || !packet.data.headers.token) {
        return;
      }

      utils.verify(packet.data.headers.token, publicKeys[_clientId[0]], (err) => {
        // Not a valid token !
        if (err) {
          return;
        }
