...
const validate = {
  ...
  buildValidateFunction : function (descriptor) {
    ...
    for (var _objName in descriptor) {
      ...
      _s += 'if (' + _varName;
      if (_objName !== 'main0') {
        _s += ' && _objectsInError.indexOf(\'' + _varParentName + '["' + _realObjName + '"]\') === -1';
      }
      _s += ') {\n';
      ...
      for (var _attr in _obj) {
        ...
        if (_code.testStr) {
          ...
          _s += '  _errors.push({"value":'+_testedVariable+', "field":"'+ _field +'", "index":'+_varNameIterator+', "error": "'+_code.errorMessage+'"});\n';
          if (_testType === 'array' || _testType === 'object') {
            _s += `
              _objectsInError.push('${ _testedVariable }');
            `;
          }
          _s += '}\n';
    ...
    if (_atLeastOneValidateFunction) {
      ...
      _s += 'if(callback){ return callback(_errors); };\n';
    }
    _s += ' return _errors;\n';
    ...
    try {
      ...
      throw new Error('kitten-mq.validate: Impossible to compile the JSON validator.\n'+err+'\n--------------------------------\n'+_s+'\n--------------------------------\n');
    }
    return _fn;
  },
  ...
  getConditionCode : function (varName, params) {
    ...
    if (!(params instanceof Array)) {
      throw new Error('kitten-mq.validate: the descriptor is invalid for the variable '+varName+'. You must use an array [] instead of '+JSON.stringify(params));
    }
    ...
    if (params.length!==0) {
      ...
      if (inlineTest[_type]) {
        ...
        throw new Error('kitten-mq.validate: Unknown descriptor type "'+_type+'" in '+varName+':'+JSON.stringify(params));
      }
    }
    else {
      return {};
    ...
    while (i < params.length) {
      ...
      switch (_argument) {
        ...
        case 'toNumber':
          ...
          break;
        case 'optional' :
          _test += ' || (' + varName + ' === null || ' + varName + ' === undefined)';
          break;
        default:
          if (typeof _argument === 'function') {
            _fnList.push(_argument);
          }
          // throw new Error('kitten-mq.validate: Unknown descriptor parameter "'+_argument+'" in '+varName+':'+JSON.stringify(params));
          break;
      }
    }
