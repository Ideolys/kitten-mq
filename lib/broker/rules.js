...
function rules (rules) {
  ...
  return {
    ...
    isAllowed : function (client, channelParts, isRead = true, tree) {
      ...
      let _cacheKey = (isRead ? 'R_' : 'W_') + client + '@' + channelParts.endpoint + '/' + channelParts.version + '/' + channelParts.id;
      ...
      if (_isExclusive && isRead) {
        ...
        for (var i = 0; i < channelParts.id.length; i++) {
          let _resIndex = index.binarySearch(tree.ids, channelParts.id[i]);
