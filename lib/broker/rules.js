...
function rules (rules) {
  ...
  return {
     ...
     * @return {Boolean}
     */
    isAllowed : function (client, channelParts, isRead = true, tree) {
      let _res              = true;
      let _isClientHasRules = false;

      let _version = channelParts.version;
      let _ids    = channelParts.ids;

      let _cacheKey = (isRead ? 'R_' : 'W_') + client + '@' + channelParts.endpoint + '/' + channelParts.version + '/' + channelParts.ids;
      ...
      if (!_rules[channelParts.endpoint][_version][channelParts.ids]) {
      ...
      if (_isExclusive && isRead) {
        if (!Array.isArray(channelParts.ids)) {
          channelParts.ids = [channelParts.ids];
        }

        for (var i = 0; i < channelParts.ids.length; i++) {
          let _resIndex = index.binarySearch(tree.ids, channelParts.ids[i]);
