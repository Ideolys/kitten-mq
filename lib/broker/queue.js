...
function queueTree (queueId) {
  let _tree = {
    // Indexes
    ids         : [], // [1               , N ]
    clientIds   : [], // [["client_1"]    , N ]
    clientNodes : [], // [see object above, N ]
    clients     : {}  // { client1 : [index_id, index_id_N] }positions where a client is in ids index
  };
  ...
  _tree.removeClient = function removeClient (client, node, type, id) {
    ...
    for (var i = _tree.clients[client].length - 1; i >= 0; i--) {
      var _clientId = _tree.ids[_tree.clients[client][i]];
      ...
      let _node  = _tree.clientNodes[_position][client].nodes;
      _indexNode = _node.indexOf(node);

      if (_indexNode !== -1) {
        _node.splice(_indexNode, 1);

        if (!_node.length) {
          _tree.clientIds[_position].splice(_indexClient, 1);
          _tree.clientNodes[_position][client].lastNode = -1;
          _tree.clients[client].splice(i, 1);
        }

        log(logger.LEVELS.DEBUG, logger.NAMESPACES.QUEUE, 'queue=' + queueId + ';from=' + node + ';unsubscribed from id=' + (id || _clientId));
      }
    }
  };
...
function queue (id, handlerForQueue, config, map = {}) {
  ...
  function _onTimeout () {
    for (let ackKey in _acks) {
      if (_acks[ackKey].created >= config.requeueInterval * 1000) {
        log(logger.LEVELS.DEBUG, logger.NAMESPACES.QUEUE, 'queue=' + id + ';resent packet messageId=' + _acks[ackKey].messageId);
        _resend(ackKey);
      }
    }
  ...
  _queue.addClient = function addClient (ids, client, node, type) {
    ...
    for (var i = 0; i < ids.length; i++) {
      ...
      if (!_res.found) {
        index.insertAt(_tree.ids, _res.index, _id);
        index.insertAt(_tree.clientIds, _res.index, []);
        index.insertAt(_tree.clientNodes, _res.index, {});
      }
      ...
      let _clientsIdsSearch = index.binarySearch(_tree.clients[client], _res.index);

      if (!_clientsIdsSearch.found) {
        index.insertAt(_tree.clients[client], _clientsIdsSearch.index, _res.index);
      ...
      if (_tree.clientNodes[_res.index][client].nodes.indexOf(node) === -1) {
        ...
        log(logger.LEVELS.DEBUG, logger.NAMESPACES.QUEUE, 'queue=' + id + ';node=' + node + ';subscribed to id=' + _id);
      }
