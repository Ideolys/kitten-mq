...
function queue (id, handlerForQueue, config, configQueue = {}) {
  ...
  /**
   * empty a queue or a specific channel in a queue
   * @param  {string} type    : should be 'primary', 'requeue' or 'secondary'
   * @param  {string} channel : optionnel, the name of a specific channel
   */
  _queue.empty = function (type, channel){
    let nbDeletedItem = 0;
    switch (type) {
      case 'primary': 
        if(channel) {
          _queue.queue = _queue.queue.filter(function(value, index, arr){
            if(value[0] === channel) {
              nbDeletedItem++;
            }
            return value[0] !== channel;
          });
        } else {
          nbDeletedItem += _queue.queue.length;
          _queue.queue = [];
        }
        break;
      case 'requeue': 
        if(channel) {
          _queue.queueRequeue = _queue.queueRequeue.filter(function(value, index, arr){ 
            if(value[1][0] === channel) {
              nbDeletedItem++;
            }
            return value[1][0] !== channel;
          });
        } else {
          nbDeletedItem += _queue.queueRequeue.length;
          _queue.queueRequeue = [];
        }
        break;
      case 'secondary': 
        for (const [key, item] of Object.entries(_queue.queueSecondary)) {
          if(typeof item === 'object' && Array.isArray(item)) {
            if(channel && channel !== key) continue;

            nbDeletedItem +=  _queue.queueSecondary[key].length;
            _queue.queueSecondary._nbMessages -= nbDeletedItem;
            _queue.queueSecondary._nbMessagesDropped +=  nbDeletedItem;
            _queue.queueSecondary[key] = [];

            stats.update({
              counterId : 'kitten_mq_queue_messages_dropped_count' + id,
              value     : _queue.queueSecondary[key].length
            });
          }
        }
        break;
