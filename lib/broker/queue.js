 ...
 */
function queue (handlerForQueue) {
  let _queue =  {
    queue          : [],
    curentItem     : null,
    nbAcks         : 0,
  ...
  function _processItemInQueue () {
    ...
    handlerForQueue(_queue.currentItem[0], _queue.currentItem[1]);
  }
   ...
   * @returns {Array}
   */
  function _getClientsToSend (tree) {
    let _clients = [];
   ...
   */
  _queue.addInQueue = function (item, tree) {
    let _clientsToSend = _getClientsToSend(tree);
