...
function queue (id, handlerForQueue, config, configQueue = {}) {
    ...
    for (let id in _queue.queueSecondary) {
      ...
      for (let i = len - 1; i >= 0; i--) {
        ...
        if (_isMessageExpired(_secondaryQueue[i])) {
          ...
          stats.update({
            counterId : 'kitten_mq_queue_messages_dead_count',
            value     : 1
          });
          stats.update({
            counterId : 'kitten_mq_queue_messages_dropped_count',
  ...
  _queue.addInQueue = function (id, item) {
    ...
    _queue.queue.push([id, item, null, Date.now()]);
  ...
  _queue.getStatistics = function getStatistics () {
    return {
      total   : stats.COUNTERS['kitten_mq_queue_messages_count'].value(),
      dropped : stats.COUNTERS['kitten_mq_queue_messages_dropped_count'].value(),
      timeout : stats.COUNTERS['kitten_mq_queue_messages_timeout_count'].value(),
      dead    : stats.COUNTERS['kitten_mq_queue_messages_dead_count'].value(),
    }
  }
