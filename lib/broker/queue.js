...
function queueTree () {
  ...
  _tree.addClient = function addClient (ids, client, node, type) {
    ...
    for (var i = 0; i < ids.length; i++) {
      let _res = index.binarySearch(_tree.ids, ids[i]);

      if (!_res.found) {
        _tree.ids[_res.index]         = [ids[i]];
  ...
  _tree.removeClient = function removeClient (client, node, type) {
    ...
    for (var i = _tree.clients[client].length - 1; i >= 0; i--) {
      ...
      let _indexClient = _tree.clientIds[_index].indexOf(client);

      if (_indexClient !== -1) {
        _tree.clientIds[_index].splice(_index, 1);
      }
...
function queue (handlerForQueue, requeueLimit, requeueInterval) {
   ...
   */
  _queue.addInQueue = function (id, item, tempQueue) {
    let _clientsToSend = _getClientsToSend(id);
