...
function createLastItems (queue) {
  ...
  lastObjectMainDiv.innerHTML      = JSON.stringify(queue.lastItem[1], null, 2);
  lastObjectSecondaryDiv.innerHTML = JSON.stringify(queue.lastItemSecondary[1], null, 2);
...
function createClientsList (queue) {
  ...
  for (client in clients) {
    ...
    let ids = clients[client].map((id) => {
      return '<span class="' + (id.isConsumer ? 'consumer' : 'listerner') + '">' + id.label + '</span>';
    }).join(', ');
