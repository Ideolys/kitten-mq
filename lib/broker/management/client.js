const menu        = ['Clients', 'Messages', 'Config'];
...
/**
 * Hander for click on menu item in queue preview
 * @param {Object} queue
...
function onClickQueuePreviewMenuItem (queue, menuItem, divContent) {
  ...
  switch (menuItem) {
    ...
    case menu[1]:
      ...
      break;
    case menu[2]:
      divContent.appendChild(renderConfig(queue));
      break;
  }
}

/**
 * Handler for click on message
 * @param {Int} index
 * @param {Div} div
 */
 ...
 * @param {Object} queue
 */
function renderConfig (queue) {
  const pre = document.createElement('pre');
  pre.textContent = JSON.stringify(queue.config, null, 2);
  return pre;
}

/**
 * Render message list pagination
 * @param {Div} divPagination
 * @param {Div} divPreview
 * @param {Int} limit
 * @param {Int} offset
 */
...
function createQueueItem (name, queue) {
  ...
  let item = createRawItem('total(queue)', queue.queueLength + queue.queueSecondaryLength);
  ...
  seeLink.textContent = 'Show queue';
  seeLink.style.color = '#2563EB';
