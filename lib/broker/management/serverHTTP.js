  ...
  return `
    ...
    <html lang="fr">
      ...
      <body dir="ltr" class="text-lg">
        ...
        <script>
          const queues          = JSON.parse('${JSON.stringify(queues)}');
...
module.exports = {
  init : function (config, queues) {
    ...
    server.listen(config.httpServerPort || 8080, (err) => {
      if (err) {
        log(logger.LEVELS.ERROR, namespace, 'Failed to start HTTP server', err);
      }

      log(logger.LEVELS.ERROR, namespace, 'HTTP server started on port ' + (config.httpServerPort || 8080), err);
    });
  }
}
