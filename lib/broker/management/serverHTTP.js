...
function createNetdataOutput (queue) {
  ...
  for (let channel in queue) {
    let counterMain             = queue[channel].nbMessagesReceived;
    let counterSecondary        = queue[channel].queueSecondary._nbMessagesReceived;
    let counterSecondaryDropped = queue[channel].queueSecondary._nbMessagesDropped;

    queue[channel].nbMessagesReceived                 = 0;
    queue[channel].queueSecondary._nbMessagesReceived = 0;
    queue[channel].queueSecondary._nbMessagesDropped  = 0;

    res += 'Channel : ' + channel + ' nbMessagesMain : ' + counterMain +' nbMessagesSecondary : ' + counterSecondary + ' nbMessagesDropped : ' + counterSecondaryDropped + '\n';
  }
  return res;
}
