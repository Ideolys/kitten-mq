...
function broker (config) {
  ...
  function _onReveiveSendAction (client, handlerId, channel, data) {
    ...
    if (_errors) {
      return _sendToClients(null, [_fullyQualifiedClient], { handlerId, data : null }, null, false, _getError(constants.ERRORS.BAD_FORMAT, _errors));
    }
  ...
  function _onReceiveAddOrRemoveId (client, handlerId, data, route) {
    ...
    if (route === routes.QUEUE_LISTEN_ADD_ID || route === routes.QUEUE_CONSUME_ADD_ID) {
      return _queues[_queueKey].addClient(_channel.id, _client[0], _clientHandler, _type);
    }
    if (route === routes.QUEUE_LISTEN_REMOVE_ID || route === routes.QUEUE_CONSUME_REMOVE_ID) {
      return _queues[_queueKey].tree.removeClient(_client[0], _clientHandler, _type, _channel.id);
    }
  }
