...
function broker (config) {
   ...
   * @param {String} error
   */
  function _sendToClients (acks, clients, item, headers, error = null) {
  ...
  function _registerQueueAndHandler (client, handlerId, channel, clientType) {
    ...
    if (_endpoint === '*') {
      return _sendToClients(null, [_fullyQualifiedClient], { handlerId, data : null }, null, constants.ERRORS.BAD_ENPOINT );
    }
    ...
    if (_version === '*') {
      ...
      if (!_isAllowed) {
        return _sendToClients(null, [_fullyQualifiedClient], { handlerId, data : null }, null, constants.ERRORS.NOT_ALLOWED );
      }
    ...
    let _isAllowed = _rules.isAllowed(_client[0], _channelParts);
    if (!_isAllowed) {
      _sendToClients(null, [_fullyQualifiedClient], { handlerId, data : null }, null, constants.ERRORS.NOT_ALLOWED );
    }
  ...
  function start (callback) {
    ...
    _sockets = sockets.start(_config, _clients, _publicKeys, _listenBroker, _listenManagement);
