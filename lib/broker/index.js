...
function broker (config) {
   ...
   */
  function _sendToClients (acks, clients, item, error = false) {
    for (var i = 0; i < clients.length; i++) {
      ...
      if (!error) {
        acks[clients[i] + '_' + _packet.headers.messageId] = 1;
      }
  ...
  function _registerQueueAndHandler (client, handlerId, channel, clientType) {
    ...
    if (_endpoint === '*') {
      return _sendToClients(null, [_fullyQualifiedClient], { handlerId, data : null }, constants.ERRORS.BAD_ENPOINT );
    }
  ...
  function _onReceiveAck (client, handlerId, messageId, channel) {
    ...
    _queue.ack(_fullyQualifiedClient + '_' + messageId);
  };
