...
function broker (configPath, isProd = false) {
  ...
  let _listenBroker = function route (packet, client) {
    ...
    if (_route === routes.QUEUE_ACK || _route === routes.QUEUE_NACK) {
      return _onReceiveAck(client, packet.data.headers.handlerId, packet.data.headers.messageId, packet.data.data.channel, _route === routes.QUEUE_NACK, _route, packet.data.data.time);
    }
