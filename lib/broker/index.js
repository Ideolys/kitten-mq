...
function broker (configPath, isProd = false) {
  ...
  let _listenBroker = function route (packet, client) {
    ...
    // Deny all messages except ack or nack
    if (_config.isStopping && _route !== routes.QUEUE_ACK && _route !== routes.QUEUE_NACK) {
      return;
    }
    ...
    if (_route === routes.QUEUE_ACK || _route === routes.QUEUE_NACK) {
      return _onReceiveAck(client, packet.data.headers.handlerId, packet.data.headers.messageId, packet.data.data.channel || packet.data.data, _route === routes.QUEUE_NACK, _route, packet.data.data.time);
    }
  ...
  function _registerQueueAndHandler (client, handlerId, channel, clientType, route) {
    ...
    if (_channel.id === '*') {
      ...
      if (!_isAllowed) {
        log(logger.LEVELS.INFO, logger.NAMESPACES.HANDLER, 'from=' + client + ';router=' + formatRoute(_channel) + ';failed to register listener/consumer, not allowed');
   ...
   * @param {String} route
   * @param {Int} time before redeliver message
   */
  function _onReceiveAck (client, handlerId, messageId, channel, isNack, route, time) {
    let _fullyQualifiedClient = client + '@' + handlerId;
    ...
    if (isNack) {
      return _queue.nack(messageId, time);
    }
  ...
  function start (options, callback) {
    ...
    if(options && options.socketName) {
      cli.init(options, _queues);
    } else {
      log(logger.LEVELS.WARN, logger.NAMESPACES.CLI, 'no socket name found to init CLI');
    }
  ...
  function _defineListenersForSignals () {
    // Listener for SIGUSR2 to reload
    process.on('SIGUSR2', reload);

    // Listener for SIGUSR1 to softstop
    process.on('SIGUSR1', () => {
      console.log('Soft stop initiated');
      console.log('-- deny new connections');
      console.log('-- deny new messages');
      console.log('-- send messages in queues');

      _config.isStopping = true;

      // Check if there are messages to send
      setInterval(() => {
        for (let queue in _queues) {
          if (_queues[queue].queue.length) {
            return;
          }
        }

        // now we can stop the broker
        stop(() => {
          stats.stopAVGStats();
