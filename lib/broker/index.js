...
function broker (config) {
  ...
  let _listenBroker = function route (packet, client) {
    ...
    if (_route === routes.QUEUE_LISTEN_ADD_ID || _route === routes.QUEUE_LISTEN_REMOVE_ID) {
      return _onReceiveListenAddOrRemoveId(client, packet.data.headers.handlerId, packet.data.data, _route);
    }
  }
  ...
  function _registerQueueAndHandler (client, handlerId, channel, clientType) {
    ...
    if (!_queues[_keyQueue]) {
      _queues[_keyQueue] = queue.queue(_sendToClients, _config.requeueLimit, _config.requeueInterval);
    }
    ...
    if (_channel.ids === '*') {
      ...
      return _tree.addClient(_channel.ids, _client[0], _clientHandler, clientType);
    }
    ...
    _addToClients(client, handlerId, _channel, clientType);
    _tree.addClient(_channel.ids, _client[0], _clientHandler, clientType);
  }
  ...
  function _onReveiveSendAction (client, handlerId, channel, data) {
    ...
    _tempQueue = _queue.addInQueue(_channel.ids, _item, _tempQueue);
   ...
   */
  function _onReceiveListenAddOrRemoveId (client, handlerId, data, route) {
    let _client        = client.split('#');
    let _channel       = {};
    ...
    if (route === routes.QUEUE_LISTEN_ADD_ID) {
      return _queues[_queueKey].tree.addClient(_channel.ids, _client[0], _clientHandler, constants.LISTENER_TYPES.LISTEN);
    }
    if (route === routes.QUEUE_LISTEN_REMOVE_ID) {
      return _queues[_queueKey].tree.removeClient(_client[0], _clientHandler, constants.LISTENER_TYPES.LISTEN, _channel.ids);
    }
  }
