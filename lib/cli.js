...
const cli = {
  ...
  deleteQueue : function (queues, options) {
    ...
    if(options.onPrimary && options.onRequeue && options.onSecondary) {
      ...
      return {action : 'delete', result : {message : `queue ${options.queue} droped`}};
    }
    let str = ''
    if(options.onPrimary) {
      queues[options.queue].empty('primary', options.channel);
      str += 'primary';
    }

    if(options.onRequeue) {
      queues[options.queue].empty('requeue', options.channel);
      if(str !== '') {
        str +=', '
      }
        str +='requeue'
    ...
    if(options.onSecondary) {
      queues[options.queue].empty('secondary', options.channel)
      if(str !== '') {
        str +=', '
      }
        str +='secondary'
    ...
    return {action : 'delete', result : {message : `queue ${options.queue} droped for ${str}`}};
...
function generateQueueObject (name, queue, channel, options) {
  ...
  if(_queue.primary.count > 0) {
    ...
    for(let i = 0; i < _queue.primary.count; i++) {
      ...
      _tmp[_item[0]].count++
      if(options.includeMessage && options.onPrimary) {
        _tmp[_item[0]].message.push({..._item[1].channel, ..._item[2], ..._item[1].data})
      }
    }
  ...
  if(_queue.primary.countRequeue > 0) {
    ...
    for(let i = 0; i < _queue.primary.countRequeue; i++) {
      ...
      _tmp[_item[0]].count++
      if(options.includeMessage && options.onRequeue) {
        _tmp[_item[0]].message.push({..._item[1].channel, ..._item[2], ..._item[1].data})
      }
    }
    for (const [name, item] of Object.entries(_tmp)) {
  ...
  for (const [key, item] of Object.entries(queue.queueSecondary)) {
    if(typeof item === 'object' && Array.isArray(item)) {
      ...
      let message = [];
      if(options.includeMessage && options.onSecondary) {
        for (let i = 0; i < item.length; i++) {
          message.push({...item[i][1].channel, messageId : item[i][2].messageId, ...item[i][1].data})
        }
      }
